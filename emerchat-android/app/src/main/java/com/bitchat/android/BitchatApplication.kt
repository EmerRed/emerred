...
class BitchatApplication : Application() {
    ...
    override fun onCreate() {
        ...
        // Telemetry uploader (fork addition); inert until enabled in Settings
        try { com.bitchat.android.services.TelemetryService.initialize(this) } catch (_: Exception) { }

        // Remote alarm channel (fork addition): WebSocket that force-enables
        // telemetry when the server declares an emergency
        try { com.bitchat.android.services.TelemetryAlarmListener.start(this) } catch (_: Exception) { }
